// src/renderer/modules/file-io.js - Адаптировано для веб-версии
console.log('[Module Loaded] file-io.js (Web Version)');
...
export function initializeFileIO(cmInstance) {
  ...
  document.getElementById('open-file-input').addEventListener('change', handleFileSelect);
...
async function handleFileSelect(event) {
  ...
  try {
    ...
    await loadFileContent(content, file.name);
    setButtonLoading(openBtn, false, 'Открыть');
    // Сбрасываем value, чтобы можно было открыть тот же файл снова
    event.target.value = '';
  } catch (error) {
