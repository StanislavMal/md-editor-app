...
function applyChanges(content, selection) {
  ...
  // Удаляем маркеры ----START и ----END, если они присутствуют
  // (дополнительная защита на случай, если AI включил маркеры в ответ)
  cleanContent = cleanContent
    .replace(/^\s*----START\s*/, '') // Убираем ----START в начале с любыми пробелами
    .replace(/\s*----END\s*$/, '') // Убираем ----END в конце с любыми пробелами
    .replace(/\s*----START\s*/g, ' ') // Убираем все вхождения ----START с заменой на один пробел
    .replace(/\s*----END\s*/g, ' ') // Убираем все вхождения ----END с заменой на один пробел
    .trim();
